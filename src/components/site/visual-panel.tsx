...
export function VisualPanel({ label, variant = "boiler" }: VisualPanelProps) {
  ...
  return (
    <div className="relative min-h-[460px] overflow-hidden rounded-[2rem] border border-black/10 bg-white shadow-2xl">
      <div className="absolute inset-x-0 top-0 h-32 bg-[#0f766e]" />
      <div className="absolute inset-x-6 top-6 rounded-[1.5rem] border border-black/10 bg-[#fff8e7] p-5 shadow-xl">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-3">
          {["Service", "Area", "Urgency"].map((item) => (
            <span key={item} className="h-12 rounded-full bg-white shadow-sm" />
          ))}
        </div>
      </div>
      <div className="absolute bottom-8 left-6 right-6 grid gap-4 sm:grid-cols-[1fr_0.82fr]">
        <div className="rounded-[1.5rem] border border-black/10 bg-slate-950 p-5 text-white shadow-xl">
          <div className="flex items-center gap-3">
            <span className="grid size-10 place-items-center rounded-full bg-amber-300 text-slate-950">
        ...
        <div className="grid gap-4">
          <div className="rounded-[1.5rem] border border-black/10 bg-white p-4 shadow-lg">
